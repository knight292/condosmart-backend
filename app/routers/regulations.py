...
def get_regulations(
    ...
    query = db.query(Regulation).filter(
        Regulation.condominium_id == current_user.condominium_id,
    ...
    for reg in regulations:
        creator = db.query(User).filter(User.id == reg.created_by).first()
        reg_dict = {
            "id": reg.id,
...
def get_regulation(
    ...
    if regulation.condominium_id != current_user.condominium_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    
    creator = db.query(User).filter(User.id == regulation.created_by).first()
    reg_dict = {
        "id": regulation.id,
